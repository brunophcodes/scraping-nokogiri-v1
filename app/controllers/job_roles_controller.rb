  ...
  def scrap_job_role(uri)
    begin
      ...
      if data != "expired"
        ...
        redirect_to :home, alert: "The job role may be expired"
      end
    rescue OpenURI::HTTPError => e
      Rails.logger.error "HTTP Error while scraping #{uri}: #{e.message}"
      # Handle the error appropriately, e.g., redirect with an error message
      return "error" # Or some other indicator of failure
    rescue StandardError => e
      Rails.logger.error "An unexpected error occurred while scraping #{uri}: #{e.message}"
      return "error"
    end
  end
